...
class Addons_spins extends Interspire_Addons
{
    ...
    {
        MT::S()->GUI($this->addon_name,$this->template_system);
		/*
        $api=$this->GetApi();
		//$api->get_random_url();
		$r=$api->url_details();
	...
	public function Admin_Action_Addurl()
	{
		...
		else 
		{
			...
			{
				$this->template_system->assign('do',"add");
			}
    ...
    static function SetMenuItems (EventData_IEM_SENDSTUDIOFUNCTIONS_GENERATEMENULINKS $data)
    {
        ...
        $menuItems['newsletter_button'][]=array (
					'text' => GetLang('Addon_spins_configuration'),
					'link' => $self->admin_url . '',
					'image' => 'triggeremails_view.gif',
					'show' => $user->isAdmin(),
					'description' => GetLang('Addon_spins_configuration_description'),
		);
        $data->data = $menuItems;
    }
    static function RegisterAddonPermissions ()
    {
        ...
        $perms = array (
            'spins' => array (
                'addon_description' => "MT: Spins Manager",
                'default' => array('name' => "Manage URL Rotation + Masquerading")
            )
        );
