...
$(function(){
	...
	check_all("selec_all","mass_mta");
	check_all("selec_all_users","mass_users");
	check_all("group_selec_all","gcheckbox");
	$(".mtas_action_go").click(mtas_action_go);
});
...
var mtas_action_go = function()
{
	$('#mass_mta_form').submit();
}
var loaded_mta_stat=[];
...
var mta_details_do = function(what,mta,data)
{
	if(what=="test")
	{
		...
		var data = $.extend({'do':"mta_test"},make_form_to_object_div('#mail_test_'+mta));
	...
	else if(what=="save")
	{
		...
		//alert('#mail_test_'+mta);
		var data = $.extend({'do':"mta_modify"},make_form_to_object_div('#mail_test_'+mta));
		$.post('index.php?Page=Addons&Addon=mta&Action=ajax&ajax=1',data, function(response) {
			var r = $.evalJSON(response);
...
var make_form_to_object_div = function(form)
{
