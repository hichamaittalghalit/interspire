						...
						<div id="GraphPanel">
							<div class="PanelHeaderBox1 IndexPage_GettingStarted_Header" style="padding:0px 0px 10px 5px;">
								<div id="HomeStartTitle"  style="border-bottom:1px solid #eee;">%%LNG_Addon_mta_accounts%%</div>
                                                        ...
                                                        <div>
                                                        <input type="button" name="AddListButton" value="%%LNG_Addon_mta_new_mta%%" class="btn btn-success" onclick="javascript: document.location='index.php?Page=Addons&Addon=mta&Action=add_mta&group_id={$group.mta_group_uid}';">
                                                        </div>
							<div>
							{if count($mtas)==0}
                                                        <br>
							...
							{else}
							<form onsubmit="return false;" name="mass_mta_form" id="mass_mta_form" method="POST" action="index.php?Page=Addons&Addon=mta&Action=mass_mta&group_id={$group.mta_group_uid}">
								<select class="mtas_action" name="mtas_action">
								...
								<input type="button" class="mtas_action_go btn" value="%%LNG_Go%%">
								<table border="0" cellspacing="0" cellpadding="0" width="100%" class="Text GroupMtas" id="SubscriberListManageList">
                					...
                					<tr class="Heading3 Heading3Center">
                					    <td width="1%" nowrap="nowrap" class="mta_checkbox">
											<input class="selec_all" type="checkbox" id="select_all">
										</td>
                						...
                						<td width="5%" nowrap="nowrap">%%LNG_Addon_mta_title_blacklist%%</td>
                                                                <td width="5%" nowrap="nowrap">%%LNG_Addon_mta_title_whitelist%%</td>
                					...
                					<tr class="GridRow {if ($key % 2)==0}GridRowWhite{else}{/if}">
                						<td nowrap="nowrap" style="border-left:1px solid #DDD;"  class="mta_checkbox"><input class="mass_mta" type="checkbox" name="mass_mta[]" value="{$item.uid}"></td>
                						...
                						{/if}
                                                                ...
                                                                <td nowrap="nowrap" class="mgs1" style="{if $item.whitelist>0}color:green;{else}{/if}">
                                                                {if $item.whitelist>0}
                                                                        <a alt="Whitelist Data" class="tooltipblacklist" style="color:green;text-decoration:none;" href="#">
                                                                                {$item.whitelist}
                                                                        </a>
                                                                        <div style="display:none">
                                                                        {if count($item.whitelist_data)==0}
                                                                                There is no new data for current Date. Try again in few minutes.
                                                                        {else}
                                                                                {foreach key=bkey item=bitem from=$item.whitelist_data}
                                                                                        {$bitem.name} (<span style="color:#666;">{$bitem.url}</span>)<br>
                                                                                {/foreach}
                                                                        {/if}
                                                                        </div>
                						...
                						{else}
                							...
                							&nbsp;&nbsp;<a style="color:red;" href="index.php?Page=Addons&Addon=mta&Action=delete_mta&group_id={$group.mta_group_uid}&id={$item.uid}" onclick="return confirm('%%LNG_Addon_mta_sure_delete_mta%%');">%%LNG_delete%%</a>
                						{/if}
                						</td>
                					</tr>
                					</tbody>
                					...
                					<tr>
                						...
                						<td colspan="14" style="padding:0px;border-right:1px solid #DDD;border-left:1px solid #DDD;">
                							<!-- MTA DETAILS BEGIN -->
